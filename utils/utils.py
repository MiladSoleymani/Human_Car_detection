    ...
    for bbox, _, class_id, tracker_id in yolo_detection:
        if class_id == 0: # do the process on persons labels
            for demography in mtcnn_detection:
                ...
                face_center = (int(region["x"] + region["w"]/2), int(region["y"] + region["h"]/2))
                if face_center[0] > bbox[0] and face_center[0] < bbox[2] and face_center[1] > bbox[1] and face_center[1] < bbox[3]: 
                    best_detection.append({"id":tracker_id, "region":region, "age":demography["age"], "dominant_gender":demography["dominant_gender"]})
